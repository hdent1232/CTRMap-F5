...
public class NpcEditFormGuardsTest {
	...
	public static void main(String[] args) throws Exception {
		...
		if (!dump.isDirectory()) {
			...
			aRefusedZoneSaveDoesNotReShowTheScript(zo);
	...
	static void removeKeepsModels(GARC zo, GARC gr) throws Exception {
		...
		NPCEditForm form = new NPCEditForm(LOADED, TOOLS, REDRAW, NAVI, CENTRE, SAVER, SAVER, CtrmapMainframe.mTileMapPanel, pane -> { });
	 ...
	 */
	/**
	 * A zone that would not save is not followed by re-showing its script.
	 *
	 * <p>WHY. Repointing a dialogue and adding a scripted NPC both write the zone
	 * and then re-read the script from it, because the script on screen belongs to
	 * the zone that was just rewritten. The write can REFUSE - a warp with no
	 * destination is the usual reason, and the user is told - and the answer was
	 * thrown away. So the editor reloaded the script from a zone that had not been
	 * written, and showed it as though it were the saved one.
	 *
	 * <p>Neither half of this could be asserted before: both the zone save and the
	 * script editor were reaches through the main window for a whole panel, to use
	 * one method on it.
	 */
	static void aRefusedZoneSaveDoesNotReShowTheScript(GARC zo) throws Exception {
		System.out.println("--- a zone that would not save is not followed by re-showing its script");
		Zone zone = openZone(zo, ZONE);
		NPCEditForm form = new NPCEditForm(LOADED, TOOLS, REDRAW, NAVI, CENTRE, SAVER, SAVER, CtrmapMainframe.mTileMapPanel, pane -> { });
		form.loadFromEntities(zone.entities, null);

		SAVER.reset();
		java.lang.reflect.Method save = form.getClass().getDeclaredMethod("saveZoneScript", Zone.class);
		save.setAccessible(true);
		save.invoke(form, zone);
		check(SAVER.saves.equals(java.util.Arrays.asList(Boolean.FALSE)),
			"the zone is written, asking the user nothing: " + SAVER.saves);
		check(SAVER.shown.size() == 1, "and the script is re-shown once it has been: " + SAVER.shown.size());

		SAVER.reset();
		SAVER.writes = false;
		save.invoke(form, zone);
		check(SAVER.saves.size() == 1, "a refused write is still attempted once");
		check(SAVER.shown.isEmpty(),
			"AND THE SCRIPT IS NOT RE-SHOWN, because it would be read from a zone that was"
			+ " never written: " + SAVER.shown.size() + " reload(s)");
		SAVER.reset();
	}
	...
	static void misnumberedUidsAskFirst(GARC zo) throws Exception {
		...
		try {
			new NPCEditForm(LOADED, TOOLS, REDRAW, NAVI, CENTRE, SAVER, SAVER, CtrmapMainframe.mTileMapPanel, pane -> { }).loadFromEntities(e, null);
		} finally {
			ctrmap.Ui.stopRecording();
		}
	...
	static void missingRegistryEntryAsksFirst(GARC zo) throws Exception {
		...
		try {
			new NPCEditForm(LOADED, TOOLS, REDRAW, NAVI, CENTRE, SAVER, SAVER, CtrmapMainframe.mTileMapPanel, pane -> { }).loadFromEntities(zone.entities, reg);
		} finally {
			ctrmap.Ui.stopRecording();
		}
