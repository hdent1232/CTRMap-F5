...
public class MapResizerTest {
	...
	public static void main(String[] args) throws Exception {
		File garcFile = new File(args.length > 0 ? args[0]
				: "C:/Users/flami/Desktop/Claude/sessions/3DS Editor/RomFS_original_garcs/a/0/4/0");
		GARC mm = new GARC(garcFile);
		int tested = 0, ok = 0, lod = 0, failures = 0;
		...
		System.out.println("\nMapResizer: tested=" + tested + " (LOD skipped " + lod + ")  ok=" + ok + "  failures=" + failures);
	...
	static void checkGrow(byte[] mat, int mi, int newW, int newH, int w, int h) {
		...
		byte[] out = MapResizer.buildResizedMatrix(mat, newW, newH, ids);
		int count = u16(out, 2);
		if (count != u16(mat, 2)) {
			throw new IllegalStateException("subfile count changed");
		}
		int nSub0 = le32(out, 4);
		if (u16(out, nSub0) != 0 || u16(out, nSub0 + 4) != newW || u16(out, nSub0 + 6) != newH) {
