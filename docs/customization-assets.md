...
- ~~**Runtime loading is not proven.**~~ **SETTLED — CONFIRMED.** The
  ...
  87 = `a/0/8/7`, **the only zero-byte archive in the whole ORAS RomFS**.
  Full chain in `customization-scope.md`. A successful *read* is still
  unobserved — no emulator trace was taken.
...
- ~~**Whether the field dress-up path in `code.bin` is reachable**~~ —
  **SETTLED.** It is reachable, and it is the *same code* as the battle path:
  ...
  (a 0x40-byte, 32-slot part-id block). Still unproven: that anything at
  runtime sets the context byte to 0 — and with `a/0/8/7` empty, nothing
  would load if it did.
- **Whether this is XY's complete catalogue** cannot be established from an
  ORAS dump alone — there is no XY dump here to diff against. What IS
  established: ORAS ships 101 swappable part models (86 of them actually
  offered by the index) and 570 part textures, across 14 dress-up slots for
  the heroine and 13 for the hero.
